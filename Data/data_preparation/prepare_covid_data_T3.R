...
covid_T3 <- read.spss("Attitudes COVID-19 vaccins_T3_IDrecoded_v2.sav")
...
covid3 <- data.frame(
  CaseID = covid_T3$CaseID,
  time = rep(3, n= 1502),
  Vacc_Aff_2 = covid_T3$Vacc_Aff_2, 
  Vacc_Aff_4 = covid_T3$Vacc_Aff_4, 
  Vacc_Aff_7 = covid_T3$Vacc_Aff_7, 
  Vacc_Cog_1 = covid_T3$Vacc_Cog_1, 
  Vacc_Cog_2 = covid_T3$Vacc_Cog_2, 
  Vacc_Cog_3 = covid_T3$Vacc_Cog_3, 
  Vacc_Cog_4 = covid_T3$Vacc_Cog_4, 
  Vacc_Cog_5 = covid_T3$Vacc_Cog_5, 
  Vacc_Beh_2 = covid_T3$Vacc_Beh_2, 
  Vacc_Beh_3 = covid_T3$Vacc_Beh_3, 
  Vacc_Oth_5 = covid_T3$Vacc_Oth_5, 
  Vacc_Oth_6 = covid_T3$Vacc_Oth_6,
  Vacc_Oth_7 = covid_T3$Vacc_Oth_7,
  Vacc_Oth_8 = covid_T3$Vacc_Oth_8
)

for(col in 2:ncol(covid3)){
  covid3[, col] <- as.numeric(covid3[, col])
}

# compute average attitude 
covid3$mean_attitude <- covid3 %>%
  select(3:13) %>%
  rowSums(.)/11

# compute average involvement 
covid3$mean_involvement <- covid3 %>%
  select(14:16) %>%
  rowSums(.)/3

# save dataframe 
write.csv(covid3,
          "Data/processed_data/covid_T3.csv")
